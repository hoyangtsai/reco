...
class InitCommand extends Command {
  ...
  * run(ctx) {
    ...
    yield this.installToolkit(targetDir);
   ...
   */
  * installPkgDeps(targetDir, pkgName) {
    const args = pkgName ? ['install', pkgName] : ['install'];
   ...
   * @memberof InitCommand
   */
  * installToolkit(targetDir) {
    const { argv } = this.context;
    this.log('Prepare to install toolkit');
    if (!argv.toolkit) {
      return this.log('toolkit is not specified, just ingore~');
    }
    // toolkit is package
    if (argv.toolkit.startsWith('@tencent')) {
      this.log('tookit is a package, just install it!');
      yield this.installPkgDeps(targetDir, argv.toolkit);
      return argv.toolkit;
    }
    this.log('toolkit is a dir, just return it!');
    // toolkit is dir
    return argv.toolkit;
  }
      ...
      fsEditor.copy(from, to, {
        process: (content) => {
          ...
          if (file === '_package.json' || file === 'package.json') {
            ...
            if (pkg.template && argv.tookit) {
              pkg.template.toolkit = !argv.toolkit.startsWith('@tencent') ? path.join(cwd, argv.toolkit) : argv.toolkit;
            }
  ...
  * findTemplateDir() {
    ...
    this.log(`use toolkit: ${argv.toolkit}`);
    templateDir = yield this.downloadBoilerplate(pkgName);
    return templateDir;
  }

  * downloadBoilerplate(pkgName) {
    const pkgInfo = yield this.helper.getPackageInfo(this.registryUrl, pkgName, false, this.logger.log);
  ...
  * fetchBoilerplateMapping(pkgName) {
    const pkgInfo = yield this.helper.getPackageInfo(this.registryUrl, pkgName || this.configName, true, this.logger.log);
